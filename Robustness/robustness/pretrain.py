...
def preprocess_for_pretrain(lines, mask_rate, in_statement_pred):
    ...
    a = before_commit+after_commit
    b = before_commit+after_commit
    if a==b:
        a_len = [len(src.split()) for src in a]
        maxlen = max(a_len)
        maxlen_idx = np.asarray(a_len).argmax()

        mask_len = math.floor(mask_rate*maxlen)
        mask_start = random.randint(0, maxlen-mask_len)
        mask_end = mask_start + mask_len
        line_to_mask = a[maxlen_idx].split()

        for i in range(mask_start, mask_end):
            line_to_mask[i] = Constants.MSK_WORD
        a[maxlen_idx] = " ".join(line_to_mask)
    ...
    source_list.append("<nl> ".join(a))
    target_list.append("<nl> ".join(b))
    ...
    # print(before_commit, after_commit, before_commit==after_commit)
