...
def preprocess_for_pretrain(lines, mask_rate, in_statement_pred):
    ...
    if before_commit == after_commit:
        before_commit_len = [len(src.split()) for src in before_commit] # [lengths of sentences in before_commit]

        maxlen = max(before_commit_len) # max value in before_commit
        maxlen_idx = np.asarray(before_commit_len).argmax() # index of max value in before_commit

        mask_len = math.floor(mask_rate*maxlen) # keep int, give 4.5, return 4
        ...
        # before_commit_mask[maxlen_idx] = " ".join(line_to_mask)
    ...
    # before_commit_nomask = copy.deepcopy(before_commit)
    # before_commit_mask= copy.deepcopy(before_commit)
    ...
    # # after_commit
    # after_commit_len = [len(src.split()) for src in after_commit] # [lengths of sentences in before_commit]
    # maxlen = max(after_commit_len) # max value in before_commit
    # maxlen_idx = np.asarray(after_commit_len).argmax() # index of max value in before_commit

    # mask_len = math.floor(mask_rate*maxlen) # keep int, give 4.5, return 4
    # mask_start = random.randint(0, maxlen-mask_len)
    # mask_end = mask_start + mask_len
    # line_to_mask = after_commit[maxlen_idx].split()

    # for i in range(mask_start, mask_end):
    #     line_to_mask[i] = Constants.MSK_WORD
    # # after_commit[maxlen_idx] = " ".join(line_to_mask)

    # after_commit_nomask = copy.deepcopy(after_commit)
    # after_commit_mask = copy.deepcopy(after_commit)
    # after_commit_mask[maxlen_idx] = " ".join(line_to_mask)

    # nomask = before_commit_nomask+after_commit_nomask
    # mask = before_commit_mask+after_commit_mask

    
    # source_list.append("<nl> ".join(mask))
    # target_list.append("<nl> ".join(nomask))
...
def main():
    ...
    # print(type(train_src_word_insts), len(train_src_word_insts))
    # ii = 0
    # for l in train_src_word_insts:
    #     if l:
    #         if "<MASK>" in l:
    #             ii+=1
    #     else:
    ...
    assert 'dict' in predefined_data #确定dict在predefined_data中，否则抛出异常
    

    print('[Info] Pre-defined vocabulary found.') #加载字典
    ...
    data = {
        ...
        'train': {
            'src': train_src_insts,
            'graph': predefined_data['train']['graph'],
            'namepre': predefined_data['train']['namepre'],
            'namepost': predefined_data['train']['namepost'],
            'graphdiff': predefined_data['train']['graphdiff'],
            'tgt': train_tgt_insts},
        'valid': {
            'src': valid_src_insts,
            'graph': predefined_data['valid']['graph'],
            'namepre': predefined_data['valid']['namepre'],
            'namepost': predefined_data['valid']['namepost'],
            'graphdiff': predefined_data['valid']['graphdiff'],
            'tgt': valid_tgt_insts}}
