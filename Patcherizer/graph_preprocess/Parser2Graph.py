...
def main():
    ...
    parser.add_argument('-savefile', type = str, default = './exp/exp/vocab/vocab')
    parser.add_argument('-vocab', type = str, default = './graph-c.png')
    ...
    _pairs = str_parent_child_pairs
    vocab = torch.load('/home/daniel/OOPSLA-1/Patcherizer/exp/exp_AST_JAVA_DIFF/vocab/vocab')
    src_word2idx = vocab['dict']['src']
    _str_parent_child_pairs = []
    _parsernames = []
    for p in str_parent_child_pairs:
        _p = []
        if p[0] in src_word2idx:
            _p.append(src_word2idx[p[0]])

        else:
            _p.append(src_word2idx[Constants.UNK_WORD])
            _parsernames.append(p[0])
        if p[1] in src_word2idx:
            _p.append(src_word2idx[p[1]])
        else:
            _p.append(src_word2idx[Constants.UNK_WORD])
            _parsernames.append(p[1])
        _str_parent_child_pairs.append(_p)
        _parsernames = list(set(_parsernames))
    ...
    if args.drawcompute=='draw':
        # graph_draw(_str_parent_child_pairs, args.savefile, args.layout)
        graph_draw(_pairs, args.savefile, args.layout)
    else:
        try:
            with open(args.savepair,'wb') as f:
                pickle.dump(_str_parent_child_pairs, f)
        except IOError:
